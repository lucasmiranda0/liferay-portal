...
public class HealthVerificationResourceImplTest {
	...
	public void testPostHealthVerification() throws Exception {
		...
		try (MockedStatic<PermissionThreadLocal>
			...
			fipsModeValidatorMockedStatic.when(
				...
				FIPSHealthCheckResult.inProgress()
			);
			...
			Mockito.verify(
				...
				(Response.StatusType)Response.Status.TOO_MANY_REQUESTS
			);
		}
	...
	@Test
	public void testPostHealthVerificationBatch() throws Exception {
		_healthVerificationResourceImpl.postHealthVerificationBatch(null, null);

		Mockito.verify(
			_responseBuilder
		).status(
			(Response.StatusType)Response.Status.METHOD_NOT_ALLOWED
		);
	}
