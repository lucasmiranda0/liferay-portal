...
public class FIPSHealthCheckAuditor {
	...
	public void audit(FIPSHealthCheckResult fipsHealthCheckResult) {
		AuditMessage auditMessage = new AuditMessage(
			...
			"0", "periodic-health-failure",
			fipsHealthCheckResult.getProviderMessage());

		JSONObject additionalInfoJSONObject = auditMessage.getAdditionalInfo();

		additionalInfoJSONObject.put(
			"failedTest", fipsHealthCheckResult.getFailedTest()
		).put(
			"fipsState", fipsHealthCheckResult.getFipsState()
		).put(
			"providerMessage", fipsHealthCheckResult.getProviderMessage()
		).put(
			"providerName", fipsHealthCheckResult.getProviderName()
		).put(
			"severity", "critical"
		);
